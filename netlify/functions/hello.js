  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      const analysis = await Promise.all(urls.map(async (url) => {
        try {
          // Замер времени загрузки (новая метрика)
          const startTime = Date.now();
          const res = await fetch(url);
          const html = await res.text();
          const loadTime = Date.now() - startTime;

          // Анализ URL (новая метрика)
          ...
          // Подсчет ключевых слов (существующий функционал)
          ...
          // Извлечение стандартных SEO-данных (существующий функционал)
          ...
          return {
            ...
            h2: h2Tags,
            alts: altTexts,
            loadTime,           // Новая метрика
            urlAnalysis,        // Новая метрика
            contentAnalysis,    // Новая метрика
            keywordStats,       // Существующий функционал
            error: null
          };
        } catch (error) {
          return { 
