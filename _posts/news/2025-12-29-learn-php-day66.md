...
<li><strong>Блоки без содержимого</strong> — можно оставить пустыми: {% raw %}<code>{% block sidebar %}{% endblock %}</code>{% endraw %}</li>
</ul>
</div>
</div>
...
<li>Вторым параметром: {% raw %}<code>{{ include('component.html.twig', {'var': value}) }}</code>{% endraw %}</li>
